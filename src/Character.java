...
public class Character extends DoubleRectangle {
	...
	public boolean isCollidingWithBlock(Point pt1, Point pt2){	
		for(int x=(int) (this.x/Tile.tileSize); x< (int) (this.x/Tile.tileSize + 3); x++) {
			for(int y=(int) (this.y/Tile.tileSize); y< (int) (this.y/Tile.tileSize + 3); y++) {
				...
				if(Component.level.block[x][y].id != Tile.air && Component.level.block[x][y].id != Tile.jared && Component.level.block[x][y].id != Tile.stars && Component.level.block[x][y].id != Tile.water && Component.level.block[x][y].id != Tile.water1 && Component.level.block[x][y].id != Tile.water2){
	...
	public boolean isCollidingWithEnemy2(){
		for(int i=1; i<=Component.mob.toArray().length-1; i++){
